...
flags.DEFINE_string('checkpoint_dir', 'checkpoints/CPM_Object', 'Directory of model checkpoints.')


addDirVecFCLayer = False
PAFSeg = False
addSegLayer = False
...
def getNetSess(data, imgH, imgW, g=None, cropCenter=None, dataAugment=True, cropPatchSize=[300,300]):
  ...
  with g.as_default():
    sample, topLeft, bottomRight, extraScale = dataPreProcess.preProcessData(data, dataAugment, False, [imgH, imgW], 255, isConstantCropPatchSize=True,
                                                                             cropPatchSize=cropPatchSize, cropCenter=cropCenter)

    imgIn = tf.expand_dims(sample[dataPreProcess.IMAGE], 0)
    imgIn = tf.stack([imgIn[:,:,:,2], imgIn[:,:,:,1], imgIn[:,:,:,0]], axis=3)

    assert imgH == imgW
    handNet = CPM(crop_size=imgH, out_chan=8, withPAF=False, numStage=3, withDirVec=addDirVecFCLayer, withConf=False, withSeg=False)
